...
void saveParallaxImagePNG(const char* filePath, int width, int height,
                          const uint8_t* pixels) {
    Timer t("saveParallaxImagePNG", -1);
    ...
    for (int y = 0; y < height; y++) {
        ...
        memcpy(rawData.data() + rowStart + 1, pixels + y * rowBytes, rowBytes);
    }
    ...
    fclose(fp);
    LOGW("Saved parallax PNG: %s (%dx%d, %u bytes)", filePath, width, height,
         (uint32_t)zlibData.size());
}
...
void renderAndSaveParallaxImages(int frameID) {
    ...
    std::thread([=]() {
        saveParallaxImagePNG(leftPath, WIDTH, HEIGHT, leftPixels.data());
    }).detach();
    ...
    std::thread([=]() {
        saveParallaxImagePNG(rightPath, WIDTH, HEIGHT, rightPixels.data());
    }).detach();
